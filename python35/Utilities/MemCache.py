...
def disk_cache_function(max_size=1024, expiration=30 * 86400):
    def wrapper(func):
        return DiskCachedFunction(func, DiskCacheDict(max_size, expiration))
    return wrapper
...
class LRUCacheDict(object):
    ...
    If this class must be used in a multi threaded environment, the option concurrent should be
    set to true. Note that the cache will always be concurrent if a background cleanup thread
    is used.
    """

    def __init__(self, max_size=1024, expiration=1 * 60, thread_clear=False, thread_clear_min_check=60,
                 concurrent=True):
    ...
    def cleanup(self):
        ...
        if (len(self.__values) > self.max_size):
            to_delete_num = len(self.__values) - self.max_size
            for k in list(self.__access_times.keys())[:to_delete_num]:
                self.__delete__(k)
...
class DiskCacheDict(object):
    ...
    def __init__(self, max_size=1024, expiration=1 * 60, thread_clear=False, thread_clear_min_check=86400,
                 concurrent=True):
        self.max_size = max_size
        self.expiration = expiration

        self.__values = set()
        self.folder = __cache__
        self.__expire_times = OrderedDict()
        self.__access_times = OrderedDict()
        self.thread_clear = thread_clear
        self.concurrent = concurrent or thread_clear
    ...
    def __setitem__(self, key, value):
        ...
        with open(decorate_filename(key), "wb") as f:
            print()
            pck.dump(value, f)
        self.__values.add(key)
        self.__access_times[key] = t
        self.__expire_times[key] = t + self.expiration
        self.cleanup()
    ...
    def __getitem__(self, key):
        ...
        self.__access_times[key] = t
        self.cleanup()
        with open(decorate_filename(key), "rb") as f:
            return pck.load(f)
        # return self.__values[key]
    ...
    def __delete__(self, key):
        if key in self.__values:
            try:
                os.remove(decorate_filename(key))
    ...
    def cleanup(self):
        ...
        if (len(self.__values) > self.max_size):
            to_delete_num = len(self.__values) - self.max_size
            for k in self.__access_times.keys()[:to_delete_num]:
                self.__delete__(k)
...
class DiskCachedFunction(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        key = repr(
            (args,
             kwargs)) + "_fn_" + self.__name__  # In principle a python repr(...) should not return any # characters.
        try:
            return self.cache[key]
        except KeyError:
            value = self.function(*args, **kwargs)
...
def decorate_filename(key):
    return os.path.join(__cache__, "{fn}.cache").format(fn = "".join([c for c in key if re.match(r'\w', c)]))